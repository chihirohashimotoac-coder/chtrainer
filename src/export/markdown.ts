...
R1 グルーピング(「グルーピング(20全体)」を狙い3本をまとめる) / R2 ブル(Bull) / R3 ナンバー(T20同一3投セットと T20→T16→T15 の1投ずつ切替セット) / R4 ダブル(D16・D20)。

- ラウンドごとに評価し、以下の4カテゴリを100点満点で採点してください(採点基準・計算方法を必ず明示すること):
  - グルーピング力: R1のセット内3投の着弾間距離(例: 3投間の最大ペア距離・平均ペア距離)から評価。命中率ではなく「まとまり」を重視してください
  - ブル精度: R2の命中率・平均誤差・インナー/アウター比率
  - ナンバー精度: R3の命中率・平均誤差。同一3投セットと1投ずつ切替セットの成績差(ターゲット切替耐性)も評価してください
